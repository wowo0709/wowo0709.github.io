...
합승으로 가는 지점을 `h`라 한다면 결국 `s -> h, h -> a, h -> b` 모두 최단거리로 움직여야 합니다. 그리고 각 h에 대해 최단거리들의 합이 최소가 되는 때를 찾으면 됩니다. 

이 문제를 풀기 위해 필요한 건 무엇일까요? **바로 각 노드 사이 최단거리입니다.**

그리고 이를 구하기 위한 알고리즘으로 유명한 **플로이드-워셜 알고리즘**이 있죠. 이 문제는 바로 플로이드-워셜 알고리즘을 이용하는 문제입니다. 

<br>

### 전체 코드

각 노드 사이의 거리를 모두 구하는 알고리즘인 플로이드-워셜 알고리즘은 따로 그래프 자료구조를 필요로 하지 않습니다. 

```python
def solution(n, s, a, b, fares):
    dist = [[float('inf') for __ in range(n+1)] for _ in range(n+1)]
    for u,v,w in fares:
        dist[u][v], dist[v][u] = w, w
    for i in range(n+1):
        dist[i][i] = 0
    for k in range(n+1): # floyd-warshall
        for i in range(n+1):
            for j in range(n+1):
    ...
    for h in range(1,n+1):
        ans = min(ans,dist[s][h]+dist[h][a]+dist[h][b])
    return ans
```
